...
// Orquestador: Exporta Excel -> Genera y guarda PDF -> Reinicia formulario
window.guardarExcelPdfYReiniciar = async function guardarExcelPdfYReiniciar() {
    try {
        // Validaciones esenciales similares a generarPDFPrincipal
        const empresa = document.getElementById('empresa')?.value || '';
        const responsable = document.getElementById('responsable')?.value || '';
        const cedula = document.getElementById('cedula')?.value || '';
        if (!empresa || !responsable || !cedula) {
            alert('Complete Empresa, Responsable y Cédula antes de guardar.');
            return;
        }

        // Validar fechas en línea si está disponible
        if (typeof validarFechasCasosContraTrimestreInline === 'function') {
            const hayErrores = validarFechasCasosContraTrimestreInline();
            if (hayErrores) {
                if (typeof mostrarMensajeValidacionGlobal === 'function') mostrarMensajeValidacionGlobal('Corrija las fechas antes de guardar.', true);
                return;
            } else if (typeof mostrarMensajeValidacionGlobal === 'function') {
                mostrarMensajeValidacionGlobal('');
            }
        }

        // 1) Exportar Excel (usa persistencia-archivos.js)
        let excelOk = false;
        if (typeof window.exportarDatosExcel === 'function') {
            try {
                await window.exportarDatosExcel();
                excelOk = true;
            } catch (e) {
                console.warn('Exportar Excel falló o fue cancelado:', e);
            }
        }

        // 2) Generar PDF silencioso y guardarlo
        const trimestreTexto = (typeof obtenerTextoTrimestre === 'function' ? obtenerTextoTrimestre() : (document.getElementById('trimestre')?.value || ''))
            .toString().replace(/\s+/g, '_');
        const fechaTexto = (document.getElementById('fecha')?.value || '').toString().replace(/[\/]/g, '-');
        const nombrePDF = `Reporte_${empresa.replace(/\s+/g, '_')}_${trimestreTexto}_${fechaTexto || new Date().toISOString().slice(0,10)}.pdf`;

        let doc = null;
        let pdfOk = false;
        try {
            if (typeof window.generarPDFSilencioso === 'function') {
                doc = await window.generarPDFSilencioso();
            } else if (typeof jspdf !== 'undefined' && typeof jspdf.jsPDF !== 'undefined') {
                const { jsPDF } = jspdf; doc = new jsPDF();
            }
            if (doc) {
                try { doc.save(nombrePDF); pdfOk = true; } catch (e) { console.warn('Fallo al guardar PDF:', e); }
            }
        } catch (e) {
            console.warn('Error generando/guardando PDF:', e);
        }

        // 3) (Opción B) Ofrecer fusionar con Excel maestro ANTES de reiniciar
        try {
            if (typeof window.actualizarExcelExistente === 'function') {
                const deseaFusionar = confirm('¿Deseas actualizar tu Excel maestro acumulando Perdidas/Productos y agregando Casos/Delincuentes?');
                if (deseaFusionar) {
                    await window.actualizarExcelExistente();
                }
            }
        } catch (e) {
            console.warn('No se pudo actualizar Excel maestro:', e);
        }

        // 4) Reiniciar para ingresar nuevos datos
        if (typeof reiniciarFormulario === 'function') {
            reiniciarFormulario();
        } else {
            // Fallback mínimo
        ...
        if (typeof mostrarNotificacion === 'function') {
            if (excelOk && pdfOk) {
                mostrarNotificacion('Excel y PDF guardados. (Si elegiste) Excel maestro actualizado. Formulario listo para nuevos datos.', 'success');
            } else if (excelOk && !pdfOk) {
                mostrarNotificacion('Excel guardado. El PDF no se pudo generar/guardar. Formulario reiniciado.', 'warning');
            } else if (!excelOk && pdfOk) {
                mostrarNotificacion('PDF guardado. El Excel no se pudo exportar. Formulario reiniciado.', 'warning');
            } else {
                mostrarNotificacion('No se pudo guardar Excel ni PDF. Formulario reiniciado de todas formas.', 'error');
            }
        }
    } catch (e) {
        console.error('Error en guardarExcelPdfYReiniciar:', e);
        alert('Ocurrió un error al guardar Excel/PDF. Revise la consola.');
    }
};

// Genera el mismo PDF pero de forma silenciosa (sin abrir ni recargar)
// Devuelve la instancia jsPDF lista para guardarse.
window.generarPDFSilencioso = async function generarPDFSilencioso() {
    // Validaciones mínimas (no se interrumpe con alertas, se retorna null si faltan)
    const empresaEl = document.getElementById('empresa');
    const responsableEl = document.getElementById('responsable');
    const cedulaEl = document.getElementById('cedula');
    if (!empresaEl || !responsableEl || !cedulaEl) return null;
    const empresa = empresaEl.value || '';
    const responsable = responsableEl.value || '';
    const cedula = cedulaEl.value || '';
    if (!empresa || !responsable || !cedula) {
        console.warn('[PDF] Falta empresa/responsable/cedula. No se genera PDF.');
        return null;
    }

    if (typeof jspdf === 'undefined' || typeof jspdf.jsPDF === 'undefined') {
        console.error('[PDF] jsPDF no está disponible en window.jspdf.jsPDF');
        return null;
    }
    const { jsPDF } = jspdf;
    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
    const tieneAutoTable = (typeof doc.autoTable === 'function');
    console.log('[PDF] jsPDF listo. AutoTable:', tieneAutoTable);
    ...
    try {
        ...
        doc.text('REGISTRO DE ACTIVIDAD CRIMINAL', 105, y, { align: 'center' });
        y += 12;
        doc.setFontSize(11);
        doc.setTextColor(0,0,0);
        doc.text('Empresa: ' + empresa, 20, y);
        // Fecha actual del sistema (no dependemos del input)
        const fechaActualTxt = formatearFechaCompleta(new Date());
        doc.text('Fecha: ' + fechaActualTxt, 120, y);
        y += 7;
        doc.text('Trimestre: ' + obtenerTextoTrimestre(), 20, y);
        y += 7;
        doc.text('Responsable de Seguridad: ' + responsable, 20, y);
        doc.text('Cédula: ' + cedula, 120, y);
        y += 15;

        // Casos
        doc.setFontSize(14);
        doc.text('CASOS DELICTIVOS OCURRIDOS EN EL TRIMESTRE', 105, y, { align: 'center' });
        y += 8;
        if (tieneAutoTable) {
            const datosCasos = obtenerDatosCasosParaPDF();
            console.log('[PDF] Casos filas:', datosCasos?.length || 0);
            doc.autoTable({
                startY: y, head: [datosCasos[0]], body: datosCasos.slice(1), theme: 'grid',
                headStyles: { fillColor: grisClaro, textColor: azulOscuro, fontStyle: 'bold', halign: 'center' },
                styles: { fontSize: 8, cellPadding: 1.5, lineWidth: 0.1, overflow: 'linebreak' },
                margin: { left: 15, right: 15 }, tableWidth: 180,
                didDrawPage: () => dibujarPiePDF(doc, fechaGeneracion)
            });
            y = doc.lastAutoTable.finalY + 20;
        } else {
            doc.setFontSize(9);
            doc.text('- Detalle de casos no disponible (falta AutoTable).', 20, y);
            y += 10;
        }

        // Pérdidas
        doc.setFontSize(14);
        doc.text('REPORTE DE PÉRDIDAS TRIMESTRALES', 105, y, { align: 'center' });
        y += 8;
        if (tieneAutoTable) {
            const datosPerdidas = obtenerDatosPerdidasParaPDF();
            console.log('[PDF] Perdidas filas:', datosPerdidas?.length || 0);
            doc.autoTable({
                startY: y, head: [datosPerdidas[0]], body: datosPerdidas.slice(1), theme: 'grid',
                headStyles: { fillColor: grisClaro, textColor: azulOscuro, fontStyle: 'bold', halign: 'center' },
            ...
            doc.setFontSize(9);
            doc.text('- Detalle de pérdidas no disponible (falta AutoTable).', 20, y);
        }

        // Página 2: Delincuentes en horizontal
        try {
            doc.addPage('a4', 'landscape');
        } catch (e) {
            console.warn('[PDF] addPage landscape falló, usando doc.addPage() por defecto:', e);
            doc.addPage();
        }
        doc.setFontSize(16);
        doc.setTextColor(30, 50, 70);
        const w = doc.internal.pageSize.getWidth();
        doc.text('DELINCUENTE(S) CAPTURADO(S)', w / 2, 15, { align: 'center' });
        if (tieneAutoTable) {
            const datosDelincuentes = obtenerDatosDelincuentesParaPDF();
            console.log('[PDF] Delincuentes filas:', datosDelincuentes?.length || 0);
            const margenLateral = 15;
            const anchoTabla = Math.max(100, Math.round(w - (margenLateral * 2)));
            doc.autoTable({
                startY: 22,
                head: [datosDelincuentes[0]],
                body: (datosDelincuentes.length > 1 ? datosDelincuentes.slice(1) : []),
                theme: 'grid',
                margin: { left: margenLateral, right: margenLateral },
                tableWidth: anchoTabla,
                halign: 'center',
                headStyles: { fillColor: [240,240,240], textColor: [30,50,70], fontStyle: 'bold', halign: 'center', fontSize: 8 },
                styles: { fontSize: 8, cellPadding: 1.1, lineWidth: 0.1, overflow: 'linebreak', halign: 'center' },
                didDrawPage: () => dibujarPiePDF(doc, fechaGeneracion)
            });
        } else {
            doc.setFontSize(9);
            doc.text('- Detalle de delincuentes no disponible (falta AutoTable).', 20, 25);
        }

        return doc; // listo para guardar
    } catch (_) {
        console.error('[PDF] Excepción al generar PDF:', _);
        return null;
    }
}

// Normaliza valores escritos en inputs de fecha (text o date) con formatos locales
function normalizarFechaInput(input) {
    if (!input) return false;
    let v = (input.value || '').trim();
    if (!v) return false;
    // Si el valor ya es AAAA-MM-DD válido
    if (/^\d{4}-\d{2}-\d{2}$/.test(v)) return false;
    let d, m, y;
    if (/^\d{2}\/\d{2}\/\d{4}$/.test(v)) { // DD/MM/AAAA
        const p = v.split('/');
        d = p[0]; m = p[1]; y = p[2];
    } else if (/^\d{2}-\d{2}-\d{4}$/.test(v)) { // DD-MM-AAAA
        const p = v.split('-');
        d = p[0]; m = p[1]; y = p[2];
    } else if (/^\d{8}$/.test(v)) { // DDMMAAAA
        d = v.substring(0,2); m = v.substring(2,4); y = v.substring(4,8);
    } else {
        return false;
    }
    // Validar rangos básicos
    const di = parseInt(d,10), mi = parseInt(m,10), yi = parseInt(y,10);
    if (!(di>=1 && di<=31 && mi>=1 && mi<=12 && yi>=1900)) return false;
    const mm = String(mi).padStart(2,'0');
    const dd = String(di).padStart(2,'0');
    // Almacenamos en formato DD/MM/AAAA para consistencia visual
    input.value = `${dd}/${mm}/${yi}`;
    return true;
}

// Determina trimestre (Q1..Q4) desde número de mes (1-12)
function mesToQuarter(mesNum) {
    if (mesNum >= 1 && mesNum <= 3) return 'Q1';
    if (mesNum >= 4 && mesNum <= 6) return 'Q2';
    if (mesNum >= 7 && mesNum <= 9) return 'Q3';
    return 'Q4';
}

// Obtiene trimestre (Q1..Q4) desde un string de fecha en formatos soportados
function quarterFromFechaString(fechaVal) {
    if (!fechaVal) return null;
    try {
        let fechaUTC;
        if (fechaVal.includes('/')) {
            const p = fechaVal.split('/');
            if (p.length >= 3) fechaUTC = new Date(Date.UTC(p[2], p[1] - 1, p[0]));
        } else if (fechaVal.includes('-')) {
            const p = fechaVal.split('-');
            if (p[0].length === 4) {
                fechaUTC = new Date(Date.UTC(p[0], p[1] - 1, p[2]));
            } else {
                fechaUTC = new Date(Date.UTC(p[2], p[1] - 1, p[0]));
            }
        } else if (fechaVal.length >= 8) {
        ...
        if (!fechaUTC || isNaN(fechaUTC.getTime())) return null;
        const mesNum = fechaUTC.getUTCMonth() + 1;
        return mesToQuarter(mesNum);
    } catch (_) {
        return null;
    }
}

// Ajusta el trimestre automáticamente según la fecha de la fila editada
function asegurarTrimestreSegunFila(fila) {
    try {
        const fechaInput = fila ? fila.querySelector('.fecha-input') : null;
        if (!fechaInput || !fechaInput.value) return;
        // Para type="date" el formato es AAAA-MM-DD
        ...
        if (val.includes('-')) {
            ...
            if (p[0].length === 4) {
                ...
                if (!isNaN(mes)) q = mesToQuarter(mes);
            } else {
                // Soporte DD-MM-AAAA
                const mes = parseInt(p[1], 10);
                if (!isNaN(mes)) q = mesToQuarter(mes);
            ...
            // Soporte DDMMAAAA
            const mes = parseInt(val.substring(2,4), 10);
            if (!isNaN(mes)) q = mesToQuarter(mes);
        }
        if (!q) return;
        ...
        console.warn('No se pudo ajustar trimestre según fila:', e);
    }
}
...
// Utilidad: construir texto de usuario para pie de PDF
function obtenerTextoUsuarioParaPDF() {
    try {
        const raw = sessionStorage.getItem('usuarioActivo');
        if (!raw) return '';
        const u = JSON.parse(raw);
        const nombre = u.nombreCompleto || u.nombre || u.username || '';
        const cedula = u.cedula ? ` — ${u.cedula}` : '';
        if (!nombre && !cedula) return '';
        return `Generado por: ${nombre}${cedula}`;
    } catch (_) {
        return '';
    }
}

// Dibuja pie de página con fecha y usuario en la página actual
function dibujarPiePDF(doc, fechaGeneracion) {
    try {
        const w = doc.internal.pageSize.getWidth();
        const h = doc.internal.pageSize.getHeight();
        const usuarioTxt = obtenerTextoUsuarioParaPDF();
        doc.setFontSize(8);
        doc.setTextColor(100, 100, 100);
        const fechaTxt = `Documento generado el ${formatearFechaHora(fechaGeneracion)}`;
        doc.text(fechaTxt, w / 2, h - 10, { align: 'center' });
        if (usuarioTxt) {
            doc.text(usuarioTxt, w / 2, h - 5, { align: 'center' });
        }
    } catch (e) {
        console.warn('No se pudo dibujar el pie de PDF:', e);
    }
}
...
function calcularTotalesCasos() {
    ...
    const totalCasosElement = document.getElementById('totalCantidad');     // ID en index.html
...
function inicializarApp() {
    ...
    if (tablaCasos) {
        tablaCasos.addEventListener('input', (event) => {
            if (event.target.matches('.cantidad-input, .cuantia-input, .denuncias-input')) {
                calcularTotalesCasos();
                // Recalcular pérdidas al cambiar cantidad o cuantía
                if (event.target.matches('.cantidad-input, .cuantia-input')) {
                    actualizarReportePerdidas();
                }
            }
        });
        tablaCasos.addEventListener('change', (event) => {
            if (event.target.matches('.fecha-input')) {
                // Normalizar formatos a DD/MM/AAAA
                normalizarFechaInput(event.target);
                const fila = event.target.closest('tr');
                asegurarTrimestreSegunFila(fila);
                validarFechasCasosContraTrimestreInline();
            } else if (event.target.matches('select.tipificacion-select')) {
                actualizarReportePerdidas();
            }
        });
    }
    ...
    // Reafirmar trimestre del mes actual tras carga completa (por si otro script lo cambió)
    setTimeout(() => {
        const trimestreSelect2 = document.getElementById('trimestre');
        if (trimestreSelect2) {
            const ahora = new Date();
            const q = `Q${Math.floor(ahora.getMonth() / 3) + 1}`;
            if (trimestreSelect2.value !== q) {
                trimestreSelect2.value = q;
                actualizarTablaPerdidasTrimestre();
            }
            actualizarReportePerdidas();
        }
...
function validarFechasCasosContraTrimestreInline() {
    ...
    filasCasos.forEach(fila => {
        const fechaInput = fila.querySelector('input.fecha-input');
        if (fechaInput) {
            ...
            if (fechaInput.value) {
                try {
                    const q = quarterFromFechaString(fechaInput.value);
                    if (!q) throw new Error('Formato de fecha no reconocido');
                    // Mapear trimestre a meses y validar
                    let mes;
                    if (/^\d{4}-\d{2}-\d{2}$/.test(fechaInput.value)) {
                        const p = fechaInput.value.split('-'); mes = parseInt(p[1],10);
                    } else if (/^\d{2}\/\d{2}\/\d{4}$/.test(fechaInput.value)) {
                        const p = fechaInput.value.split('/'); mes = parseInt(p[1],10);
                    } else if (/^\d{2}-\d{2}-\d{4}$/.test(fechaInput.value)) {
                        const p = fechaInput.value.split('-'); mes = parseInt(p[1],10);
                    } else if (/^\d{8}$/.test(fechaInput.value)) {
                        mes = parseInt(fechaInput.value.substring(2,4),10);
                    }
...
function agregarValidacionInlineFechasCasos() {
    ...
    tabla.addEventListener('change', function(event) {
        if (event.target && event.target.matches('input.fecha-input')) {
            validarFechasCasosContraTrimestreInline();
        }
    });
}
...
function actualizarTablaDelincuentes() {
    ...
    window.delincuentes.forEach((delincuente, index) => {
        ...
        fila.innerHTML = `
            ...
            <td><input type="text" class="form-control cedula-input" value="${delincuente.cedula || ''}" onchange="actualizarDelincuente(${index}, 'cedula', this.value)"></td>
    ...
    filaNueva.innerHTML = `
        ...
        <td><input type="text" class="form-control cedula-input" id="nuevoCedula" placeholder="Cédula"></td>
...
function reiniciarFormulario() {
    ...
    if (fecha) {
        // Poner fecha actual legible
        try {
            fecha.value = formatearFechaCompleta(new Date());
        } catch { fecha.value = ''; }
    }

    // 2) Casos delictivos: dejar una fila vacía y limpiar totales
    const tablaCasos = document.getElementById('tablaCasosDelictivos');
    if (tablaCasos) {
        const tbody = tablaCasos.querySelector('tbody');
        if (tbody) {
            // Eliminar todas las filas excepto la primera editable (no total-row ni placeholder)
            const filas = Array.from(tbody.children);
            filas.forEach((tr, idx) => {
                if (tr.classList.contains('total-row') || tr.id === 'placeholder-casos') return;
                if (idx === 0) {
                    // Limpiar inputs/selects de la primera fila
                    tr.querySelectorAll('input, textarea').forEach(inp => inp.value = '');
    ...
    // 3) Pérdidas: limpiar filas y totales visuales si aplica
    const tablaPerdidas = document.getElementById('tablaPerdidas');
    if (tablaPerdidas) {
        const tbodyP = tablaPerdidas.querySelector('tbody');
        if (tbodyP) tbodyP.innerHTML = '';
    }
    const totalCasosPerdidas = document.getElementById('totalCasosPerdidas');
    if (totalCasosPerdidas) totalCasosPerdidas.textContent = '0';
    ...
    try {
        if (Array.isArray(window.delincuentes)) {
            window.delincuentes.length = 0; // vaciar
        } else {
            window.delincuentes = [];
        }
        if (typeof actualizarTablaDelincuentes === 'function') {
            actualizarTablaDelincuentes();
        } else if (typeof renderizarTablaDelincuentesSimple === 'function') {
            renderizarTablaDelincuentesSimple();
        }
    } catch (e) { console.warn('No se pudo reiniciar tabla de delincuentes:', e); }

    // 5) Limpieza de temporales mínimos
    try {
        localStorage.removeItem('casosTemporal');
        localStorage.removeItem('formularioTemporal');
        // NO tocar 'delincuentesPersistentes' ni 'productosRobados' (Top 20)
    } catch {}

    console.log('Formulario reiniciado totalmente (se conservan historiales y Top 20).');
}
...
async function generarPDFPrincipal() {
    ...
    try {
        ...
        const grisClaro = [240, 240, 240];
        doc.setFont('helvetica');
        // Fecha de generación para todo el documento
        const fechaGeneracion = new Date();
        ...
        doc.autoTable({
            ...
            didDrawPage: function() {
                 dibujarPiePDF(doc, fechaGeneracion);
            }
        });
        ...
        // El pie de página se dibuja en cada página vía didDrawPage de las tablas

        // Segunda página en horizontal: Delincuentes Capturados
        try {
            // Agregar nueva página en orientación horizontal
            doc.addPage('a4', 'landscape');

            // Título de la sección
            doc.setFontSize(16);
            doc.setTextColor(30, 50, 70);
            const anchoPag = (doc.internal.pageSize && typeof doc.internal.pageSize.getWidth === 'function')
                ? doc.internal.pageSize.getWidth()
                : (doc.internal.pageSize && doc.internal.pageSize.width)
                    ? doc.internal.pageSize.width
                    : doc.internal.pageSize.getWidth();
            const altoPag = doc.internal.pageSize.getHeight();
            doc.text('DELINCUENTE(S) CAPTURADO(S)', anchoPag / 2, 15, { align: 'center' });

            // Obtener datos y renderizar tabla
            const datosDelincuentes = obtenerDatosDelincuentesParaPDF();
            const cuerpo = (Array.isArray(datosDelincuentes) && datosDelincuentes.length > 1)
                ? datosDelincuentes.slice(1)
                : [];

            // Opción estable: márgenes iguales y ancho exacto = ancho de página - 30mm
            const margenLateral = 15; // mm a cada lado
            const anchoTabla = Math.max(100, Math.round(anchoPag - (margenLateral * 2)));
            ...
            doc.autoTable({
                ...
                headStyles: { fillColor: [240, 240, 240], textColor: [30, 50, 70], fontStyle: 'bold', halign: 'center', fontSize: 8 },
                styles: { fontSize: 8, cellPadding: 1.1, lineWidth: 0.1, overflow: 'linebreak', halign: 'center' },
                // Sin columnStyles fijos: AutoTable calculará el ancho real y centrará con precisión
                didDrawPage: function () {
                    // Pie de página para la página horizontal
                    dibujarPiePDF(doc, fechaGeneracion);
                }
            });
        } catch (e) {
            console.warn('No se pudo generar la sección de Delincuentes Capturados:', e);
        }
...
function obtenerDatosCasosParaPDF() {
    ...
    filas.forEach(fila => {
        ...
        const tipificacion = fila.querySelector('.tipificacion-input')?.value
            || fila.querySelector('.tipificacion-texto')?.textContent?.trim()
            || fila.querySelector("input[name='tipificacion[]']")?.value
...
function obtenerDatosDelincuentesParaPDF() {
    const datos = [];
    datos.push(['Nombre y Apellido', 'Cédula', 'Edad', 'Dirección', 'Vehículo', 'Placa', 'Color', 'Fecha Captura', 'Delito', 'Productos', 'Cuantía (B/.)', 'N° Denuncia']);
    ...
    if (window.delincuentes && Array.isArray(window.delincuentes)) {
        window.delincuentes.forEach(d => {
            // Normalizar campos provenientes de distintas fuentes/scripts
            const color = d.color || d.colorVehiculo || '';
            const productos = d.productos || d.mercancias || '';
            // cuantía puede venir como "cuantia" o "monto" (a veces con prefijo "B/. ")
            let cuantia = d.cuantia != null ? String(d.cuantia) : (d.monto != null ? String(d.monto) : '');
            cuantia = cuantia.replace(/^B\/\.\s*/i, '');
            // fecha puede venir como d.fecha (YYYY-MM-DD o DD/MM/AAAA) o d.fechaCaptura (ISO)
            let fechaRaw = d.fecha || d.fechaCaptura || '';
            if (fechaRaw && typeof fechaRaw === 'string' && fechaRaw.includes('T')) {
                // ISO -> tomar solo la parte de fecha
                fechaRaw = fechaRaw.split('T')[0];
            }
            const fechaFmt = formatearFecha(fechaRaw) || '';

            // Añadir solo si hay algún dato relevante
            if (d.nombreCompleto || d.cedula || productos || color || fechaRaw) {
                datos.push([
                    d.nombreCompleto || '',
                    d.cedula || '',
                    d.edad || '',
